from PyQt5.QtCore import Qt, QTimer, QRectF, QMetaObject, Q_ARG, QThread
    ...
    def __init__(self, message, bg_color=QColor(11, 18, 24, 230)):
        ...
        if isinstance(message, dict):
            ...
            if 'weapon' in details:
                weapon = next((line.split(': ')[1] for line in details.split('\n') if '🔫 Weapon:' in line), '')
                if weapon:
...
class ToastManager:
    ...
    def _position_toasts(self):
        ...
        positions = {
            ...
            "Top Right": (screen.width() - self.toasts[0].width() - self.spacing, self.spacing),
            "Bottom Left": (self.spacing, screen.height() - self.spacing),
            "Bottom Right": (screen.width() - self.toasts[0].width() - self.spacing, screen.height() - self.spacing),
            "Left Middle": (self.spacing, (screen.height() - self.toasts[0].height()) // 2),
            "Right Middle": (screen.width() - self.toasts[0].width() - self.spacing, (screen.height() - self.toasts[0].height()) // 2)
        }
